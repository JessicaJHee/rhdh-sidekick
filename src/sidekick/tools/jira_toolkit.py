"""
Custom Jira tools for the sidekick CLI application.

This module provides Jira integration tools using the @tool decorator
...
def _get_jira_client() -> JIRA:
    ...
    try:
        if username and token:
            ...
            client = JIRA(server=server_url, basic_auth=(username, token))
        else:
            logger.debug("Using token auth")
            client = JIRA(server=server_url, token_auth=token)
...
def _extract_github_pr_urls(text: str) -> list[str]:
    ...
    Returns:
        List of unique GitHub PR URLs found
    """
    if not text:
        return []
    ...
    matches = re.findall(github_pr_pattern, text)
    return list(set(matches))  # Remove duplicates


def parse_json_to_jira_issue(json_content: str) -> JiraIssueData | None:
    """
    Parse JSON string content into a JiraIssueData object using Pydantic.

    Args:
        json_content: JSON string that may contain Jira issue data

    Returns:
        JiraIssueData object if parsing successful, None otherwise
    """
    try:
        # Use Pydantic's built-in JSON parsing with validation
        issue_data = JiraIssueData.model_validate_json(json_content)
        logger.debug(f"Successfully parsed JSON to JiraIssueData for {issue_data.key}")
        return issue_data

    except Exception as e:
        logger.debug(f"Failed to parse JSON content as JiraIssueData: {e}")

        # Add debugging output to understand the JSON structure
        logger.debug(f"JSON content length: {len(json_content)}")
        logger.debug(f"JSON content preview (first 200 chars): {json_content[:200]}")
        logger.debug(f"JSON content preview (last 200 chars): {json_content[-200:]}")
        ...
        try:
            ...
            if hasattr(json_err, "pos"):
                ...
                end = min(len(json_content), json_err.pos + 50)
                logger.debug(f"Problematic area around position {json_err.pos}: {json_content[start:end]}")

        # Try to extract just the JSON portion if there's trailing content
        try:
            # Find the first { and count braces to find the end of the JSON object
            ...
            logger.debug(f"Extracted JSON portion: {len(json_str)} characters")

            # Try parsing with Pydantic again
            issue_data = JiraIssueData.model_validate_json(json_str)
            logger.debug(f"Successfully parsed extracted JSON to JiraIssueData for {issue_data.key}")
            return issue_data

        except Exception as e3:
            logger.debug(f"Failed to extract and parse JSON portion: {e3}")
            return None


def _format_issue_details(issue: Issue) -> JiraIssueData:
    """
    ...
    pr_urls = _extract_github_pr_urls(details["description"])
        ...
        if hasattr(issue.fields, "comment") and hasattr(issue.fields.comment, "comments"):
            ...
            for comment in comments:
                if hasattr(comment, "body") and comment.body:
                    comment_pr_urls = _extract_github_pr_urls(comment.body)
...
@tool(
    name="jira_get_issue",
    description="Get detailed information about a Jira issue including GitHub PR links and all comments",
    show_result=True,
    cache_results=True,
    cache_ttl=300,  # 5 minutes cache
)
def jira_get_issue(issue_key: str, include_all_comments: bool = True) -> str:
    """
    ...
    try:
        ...
        issue_details = _format_issue_details(issue)
...
def jira_search_issues(jql_query: str, max_results: int = 50) -> str:
    """
...
def jira_create_issue(
    ...
    try:
        ...
        server_url = os.getenv("JIRA_URL", "")
        issue_url = f"{server_url}/browse/{new_issue.key}"
